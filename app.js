...
function render(){
 ...
 const shown=projects.filter(p=>(f==='all'||p.type===f)&&(!q||[p.name,p.description,p.label,...(p.tags||[])].join(' ').toLowerCase().includes(q)));
 grid.innerHTML=shown.map(p=>`<article class="project" onclick="openProject('${encodeURIComponent(p.slug||p.name)}')"><div class="project-icon">${p.icon||'◇'}</div><h3>${escapeHtml(p.name)}</h3><p>${escapeHtml(p.description||'Open-source project')}</p><div class="project-meta"><span class="pill">${escapeHtml(p.label)}</span><span class="pill">OPEN SOURCE</span>${(p.tags||[]).slice(0,2).map(t=>`<span class="pill">${escapeHtml(t)}</span>`).join('')}</div></article>`).join('');
